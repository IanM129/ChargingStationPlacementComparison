...
def netToGraph(net_xml_filepath, internal_lengths=True, save_position=True):
    net = sumolib.net.readNet(net_xml_filepath)
    net_internal = sumolib.net.readNet(net_xml_filepath, withInternal=True)
    G = nx.DiGraph()
    if (save_position or internal_lengths):
        attrs = {}
    ...
    for edge in net.getEdges():
        ...
        G.add_edge(from_id, to_id, id=edge.getID(), length=edge.getLength())
    return G
